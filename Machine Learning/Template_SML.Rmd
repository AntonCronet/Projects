...
author: "Anton Cronet (224220895), Erik Füßlein (220241605)"
output:
  html_document:
    fig_width: 10
    theme: spacelab
...
# Put your dataset in the same folder as your R file. This code will set your working directory for this notebook to the folder where the R file is stored. This way I can rerun your code without modifications. => relative paths in the file should ensure automatic loading when running
...
```
Model insight:
10-fold cross-validation is apparently default in rpart package. The anova method uses mse for by default cv. The model would overfit (cv-error >> training-error), limiting tree depth to 5 levels kept the error rates for cv the same, while reducing the gap, leading to a non-overfit model.
